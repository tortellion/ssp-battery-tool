...
const BANNED_RE = [/\d[\d,.]*\s*(wh\/?kg|watt.?hours?\s+per\s+kilo)/i, /\b\d+(\.\d+)?c\s*(discharge|charge|rate)/i];
...
export function validateConfigResponse(parsed) {
  ...
  parsed.configurations.forEach((c, i) => {
    ...
    if (wb.bms_g === undefined || wb.bms_g === null) errors.push(`${pre}.bms_g must be provided`);
    if (!wb.thermal_g || wb.thermal_g <= 0) errors.push(`${pre}.thermal_g must be >0`);
    const notes = (c.notes || '').toLowerCase();
    // M-5: require actual numeric values adjacent to dimension keywords, not just substrings
    const hasNumericDim = (keyword) => new RegExp(keyword + '[^.\\n]{0,30}\\d|\\d[^.\\n]{0,30}' + keyword).test(notes);
    if (!hasNumericDim('length') || !hasNumericDim('width') || !hasNumericDim('depth'))
      errors.push(`${pre}.notes missing dimension arithmetic (must show numeric values for length, width, depth)`);
  });
